...
function Start-ReportExportConsole {
  [CmdletBinding()]
  param()
  try {
    ...
    if (-not (Test-Path $exportScript)) { throw "Cannot find export_reports.ps1 next to this module (at $PSScriptRoot)." }
    ...
    if ($ans -match '^(y|yes)$') {
      ...
      if (-not (Test-Path $modulePath)) { throw "Cannot find PBIXtoPBIP_PBITConversion.psm1 next to this module." }
...
Export-ModuleMember -Function Start-ReportExportConsole
